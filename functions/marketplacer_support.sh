...
function cc_menu_remove {
  local branch="${1:-$(git_current_branch)}"
  if cc_menu_present "$branch"; then
    cc_menu_initialize $(cc_menu_list -e $branch)
  fi
}
...
function cc_menu_add_item {
  ...
  defaults write net.sourceforge.cruisecontrol.CCMenu Projects -array-add "
    {
      displayName = $branch;
      projectName = \"Marketplacer - remote ($branch)\";
      serverUrl = \"https://cc.buildkite.com/marketplacer/marketplacer-remote.xml?access_token=$CC_BUILDKITE_TOKEN&branch=$branch\";
    }
  "
}
...
function cc_menu_initialize {
  ...
  defaults write net.sourceforge.cruisecontrol.CCMenu Projects -array-add "{
      displayName = \"- deploys -\";
      projectName = deploys;
      serverUrl = \"https://cc.buildkite.com/marketplacer/deploys.xml?access_token=$CC_BUILDKITE_TOKEN\";
  }"
  defaults write net.sourceforge.cruisecontrol.CCMenu Projects -array-add "{
      displayName = \"- master -\";
      projectName = \"Marketplacer - remote (master)\";
      serverUrl = \"https://cc.buildkite.com/marketplacer/marketplacer-remote.xml?access_token=$CC_BUILDKITE_TOKEN&branch=master\";
  }"
  for branch in "$@"; do
    cc_menu_add_item "$branch"
  done
...
function cc_menu_list {
  defaults read net.sourceforge.cruisecontrol.CCMenu Projects | grep displayName | tr -d '",;' | colrm 1 22 | grep -vF -e master -e deploys -e ---
}
