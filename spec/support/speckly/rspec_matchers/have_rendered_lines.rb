...
module Speckly
  RSpec::Matchers.define :have_rendered_lines do |expected, clear_style: false, wait: Speckly.default_max_wait_time|
    match do |actual|
      @original_actual = actual
      @original_expected = expected

      Speckly::Eventually.satisfy(wait: wait, break_if_stopped: actual) do
        values_match?(expected, (@actual = TTYString.parse(actual.to_s, clear_style: clear_style).split("\n")))
      end
    end
